...
def nam_sku(ma_hang: str) -> int:
    try:
        digit = ma_hang[5]
        return 2020 + int(digit)
...
def danh_gia_fn(row):
    ...
    GOI_Y = {
        ...
        'HÀNG MỚI':    'Tiếp tục theo dõi',
    }

    hang_moi = (row['ton_dau_3t'] == 0 and row['nhap_3t'] > 0)
    if hang_moi:
        return 'HÀNG MỚI', 'Mới nhập trong 3 tháng', GOI_Y['HÀNG MỚI']
...
def run_analysis(file_bytes):
    ...
    def calc_dc_row(ma_hang, ton_pv):
        ...
        for sd in SIZE_DIGITS:
            ton = int(ton_pv.loc[ma_hang, f'ton_{sd}']) if ma_hang in ton_pv.index else 0
            result[f'dc_{sd}'] = max(0, ton - 1)
        return result

    def build_sheet_data(mahang_list, is_dc=False):
        if not mahang_list:
            return pd.DataFrame()
        ...
        if is_dc:
            dc_rows = [calc_dc_row(mh, ton_pv) for mh in final['ma_hang']]
            dc_df   = pd.DataFrame(dc_rows, index=final.index)
            final   = pd.concat([final, dc_df], axis=1)
            ...
            final['dc_tong'] = final[[f'dc_{sd}' for sd in SIZE_DIGITS]].sum(axis=1)
        else:
    ...
    def co_sku_cu(ma_hang, df_raw):
        skus = df_raw[df_raw['ma_hang'] == ma_hang]['ma_sku'].tolist()
        return any(nam_sku(str(s)) <= 2023 for s in skus)

    mh_dc = agg[agg['danh_gia']=='ĐIỀU CHUYỂN']['ma_hang'].tolist()
    mh_nt = [
        mh for mh in agg[agg['danh_gia']=='BÁN CHẠY']['ma_hang'].tolist()
        if not co_sku_cu(mh, df6r)
    ]
    data_dc = build_sheet_data(mh_dc, is_dc=True)
    data_nt = build_sheet_data(mh_nt, is_dc=False)
    ...
    ws3.cell(
        2,
        1,
        f'Đánh giá dựa trên ST3M, ST6M và MOS (Months Of Stock) | ST = Bán/(Bán+Tồn) | MOS = Tồn hiện tại ÷ Bán TB tháng | Ngày: {TODAY}'
    ...
    TON_COL = {sd: 12+i for i,sd in enumerate(SIZE_DIGITS)}
    BS_COL  = {sd: 22+i for i,sd in enumerate(SIZE_DIGITS)}
    DATA_ROW = 7

    def build_phieu_sheet(wb, sheet_name, title_text, bg_title, df_data):
        ws = wb.create_sheet(sheet_name)
        title_row(ws,1,1,32,title_text,bg_title,rh=30)
        ws.merge_cells('A2:AF2')
        ws.cell(2,1,f'Ngày lập: {TODAY} | Cửa hàng: Giày Tuấn').font=F(size=9,color='555555')

        if sheet_name == 'DE_XUAT_DIEU_CHUYEN':
            txt_size = 'Điều chuyển / Đề xuất theo size'
            txt_tong = 'Tổng điều chuyển'
        else:
            txt_size = 'Bổ sung / Đề xuất theo size'
            txt_tong = 'Tổng bổ sung'

        # Header row 3 (nhóm cột)
        for txt,c1,c2,bg in [
            ('STT',1,1,C_BLUE),('Nhóm hàng',2,2,C_BLUE),('Mã hàng',3,3,C_BLUE),
            ('N-X 3 tháng',4,7,bg_title),('N-X 6 tháng',8,11,C_BLUE),
            ('Chi tiết tồn kho theo size',12,21,'2F5496'),
            (txt_size,22,31,'375623'),
            (txt_tong,32,32,'7F6000')]:
        ...
        h4=['STT','Nhóm hàng','Mã hàng','Tồn đầu','Nhập','Xuất','Tồn cuối',
            'Tồn đầu','Nhập','Xuất','Tồn cuối']
        h4 += [str(SIZE_MAP[s]) for s in SIZE_DIGITS]  # size 35-44
        h4 += [str(SIZE_MAP[s]) for s in SIZE_DIGITS]  # đề xuất theo size
        h4 += ['Tổng']
        widths=[5,18,20]+[9]*8+[7]*10+[7]*10+[10]
        ...
        for ri,row in enumerate(df_data.itertuples(),0):
            ...
            dc(ws,er,2,row.nhom,bg=bg,al='left')
            dc(ws,er,3,row.ma_hang,bg=bg,al='left')
            dc(ws,er,4,row.ton_dau_3t,bg=bg)
            dc(ws,er,5,row.nhap_3t,bg=bg)
            dc(ws,er,6,row.xuat_3t,bg=bg)
            dc(ws,er,7,row.ton_cuoi_3t,bg=bg,bold=True)
            dc(ws,er,8,row.ton_dau_6t,bg=bg)
            dc(ws,er,9,row.nhap_6t,bg=bg)
            dc(ws,er,10,row.xuat_6t,bg=bg)
            dc(ws,er,11,row.ton_cuoi_6t,bg=bg,bold=True)
            ...
            dc(
                ...
                bg='FFF2CC',
                bold=True,
                color=(C_RED if tong_val > 0 else '000000')
            )
