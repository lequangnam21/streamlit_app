...
# ─────────────────────────────────────────────────────────────
def danh_gia_fn(row):
    ton = row['ton_3t']; ban3 = row['ban_3t']
    st3 = row['st3m'];   st6  = row['st6m']
    mos = row['mos3m']
    ...
    if   st3 >= 0.60 and mos < 1.5: dg = 'BÁN CHẠY'
    elif st3 >= 0.45 and mos < 3:   dg = 'BÁN KHÁ'
    elif 3 <= mos <= 6:              dg = 'BÁN VỪA'
    else:                            dg = 'BÁN CHẬM'
...
def run_analysis(file_bytes):
    ...
    def calc_dc_row(ma_hang, ton_pv):
        # Điều chuyển = tất cả tồn theo size (giữ lại 0)
        return {f'dc_{sd}': int(ton_pv.loc[ma_hang, f'ton_{sd}'])
                if ma_hang in ton_pv.index else 0 for sd in SIZE_DIGITS}
    ...
    def calc_bs_row(ma_hang, ton_pv, si_pv):
        ...
        for sd in SIZE_DIGITS:
            ...
            if ton == 0 and si == 0: result[f'bs_{sd}'] = 0; continue
            min_bs = max(0, 2 - ton)
    ...
    ACT_COL = {sd: 23+i for i,sd in enumerate(SIZE_DIGITS)}   # col 24-33
    TONG_COL = 34
    DATA_ROW = 6
