...
def run_analysis(file_bytes):
    ...
    # ĐIỀU CHUYỂN: ưu tiên tồn nhiều nhất trong nhóm
    mh_dc = top_n_per_group(agg, 'ĐIỀU CHUYỂN', 'ton_3t', ascending=False)

    # NHẬP THÊM:
    # Tầng 1: BÁN CHẠY + năm 2024-2026
    # Tầng 2: nếu nhóm không có mã 2024-2026 thì lấy BÁN CHẠY + năm 2023
    # Không lấy 2020-2022
    ban_chay = agg[agg['danh_gia'] == 'BÁN CHẠY'].copy()

    def extract_year(ma_hang):
        try:
            code = str(ma_hang).split('-/-')[0]
            return int(code[-1]) + 2020
        except:
            return 0

    ban_chay['nam_hang'] = ban_chay['ma_hang'].apply(extract_year)

    mh_nt = []
    for nhom, grp in ban_chay.groupby('nhom'):
        grp_2426 = grp[grp['nam_hang'].between(2024, 2026)]
        if not grp_2426.empty:
            mh_nt.extend(
                grp_2426.sort_values(['st3m','ban_3t'], ascending=[False,False])['ma_hang'].tolist()
            )
        else:
            grp_2023 = grp[grp['nam_hang'] == 2023]
            mh_nt.extend(
                grp_2023.sort_values(['st3m','ban_3t'], ascending=[False,False])['ma_hang'].tolist()
            )
    ...
    ACT_COL = {sd: 22+i for i,sd in enumerate(SIZE_DIGITS)}   # col 22-31
    TONG_COL = 34
    DATA_ROW = 6
    ...
    def build_phieu(wb, sheet_name, title_text, bg_title, df_data, is_dc):
        ...
        groups = [
            ...
            (act_label,     22, 33, '1A5276'),
            (tong_label,    34, 34, '7D6608'),
        ]
